...
var files = ["vietnam-recordings.mp3", // 0
			"ted.mp3", // 1
			"the-american-dream-is-dead.m4a", // 2
			];
...
function mouseUp(e) {
   ...
   var timelineWidth = $(".timeline").width();
	...
	if (onplayhead != null) {
		...
		// change current time
		ao.audio.currentTime = ao.audio.duration * clickPercent(e, timeline, timelineWidth);
...
AudioObject.prototype.timeUpdate = function () {
  ...
  var timelineWidth = $(".timeline").width();
	...
	var ao = audioList[getAudioListIndex(this.id)];
	var playPercent = timelineWidth * (ao.audio.currentTime / ao.duration);
